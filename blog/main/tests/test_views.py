...
class userLogInAndOutTest(TestCase):
    ...
    def test_login_and_logout(self):
        ...
        response = self.client.post(
            reverse('main:user_login'), **self.credentials)
...
class blog_Test(TestCase):
    ...
    def test_update_proc(self):
        ...
        response = self.client.post(
            reverse('main:blog_proc'), post_data_before_update)
        blog = Blog.objects.get(title='TitleV1')
        post_updated_data = {
                'bid': blog.bid,
                'title': blog.title,
        ...
        self.client = Client()
        response = self.client.post(
            reverse('main:update_proc'), post_updated_data)
    ...
    def test_delete_blog(self):
        ...
        session.save()
        response = self.client.get(
            reverse('main:delete_blog'), post_delete_data)
        self.assertRedirects(response, reverse('index'))
        blog_count = Blog.objects.filter(title='TitleV1').count()
    ...
    def test_update_blog_form_success(self):
        ...
        response = self.client.get(
            reverse('main:update_blog_form'), post_update_blog_form_data)
